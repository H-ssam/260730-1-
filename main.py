...
st.markdown("---")
st.header("📝 결론: 이 예측, 얼마나 믿을 수 있을까?")

# 같은 미래 연도를 놓고 계산 방식만 바꿔보면 결과가 얼마나 달라지는지 직접 비교합니다.
alt_target_total = float(sum(cagr_forecast(g, target_year_pop) for _, g in yearly.groupby("시군구코드")))
alt_growth_rate = (alt_target_total / baseline_total - 1) * 100 if baseline_total else 0.0
diff_pct_point = alt_growth_rate - growth_rate

st.markdown(
    f"""
같은 **{target_year_pop}년**을 예측해도, 계산 방식만 바꾸면 결과가 이렇게 달라집니다.

| 계산 방식 | {target_year_pop}년 예측 전국 총인구 | {BASE_YEAR}년 대비 증감률 |
| --- | --- | --- |
| ✅ 전국 합계 추세로 직접 계산 (현재 이 앱이 쓰는 방식) | {target_total:,.0f}명 | {growth_rate:+.2f}% |
| ⚠️ 시군구별로 각각 계산 후 합산 | {alt_target_total:,.0f}명 | {alt_growth_rate:+.2f}% |

두 방식의 차이는 **{diff_pct_point:+.2f}%p**입니다. 같은 데이터를 가지고도 "추세를 어느 단위로,
어떻게 연장하느냐"에 따라 결론이 뒤집힐 수 있다는 걸 보여줍니다.
"""
)
...
- **단순 추세 연장일 뿐입니다.** 선형회귀·CAGR 모두 최근 몇 년의 흐름을 그대로 미래로 복사하는 방식이라,
  출산율 변화, 사망률, 국내외 인구 이동, 정책(신도시 개발·지역균형발전 등) 같은 실제 변화 요인은 전혀 반영하지 못합니다.
- **예측 기간이 길수록 오차가 커집니다.** 지금은 {latest_year}년 대비 {target_year_pop - latest_year}년 뒤를 내다보고 있는데,
  기간이 길어질수록(특히 20~30년 뒤) 불확실성이 급격히 커져 참고용 이상으로 신뢰하기 어렵습니다.
- **집계 단위(전국 vs 지역 합산)에 따라 결과가 달라집니다.** 위 표에서 보듯, 소수의 급성장·급감 지역이
  전국 합계를 왜곡할 수 있어 지역별 예측을 단순히 더한 값은 특히 주의가 필요합니다.
- **관측 연도 수가 적으면 추세 자체가 불안정합니다.** 데이터가 2~3개년뿐인 지역은 이상치 하나에도
  추세선이 크게 흔들립니다.
- **행정구역 통합·분리, 통계 기준 변경** 등 데이터 자체의 불연속성도 예측 오차의 원인이 될 수 있습니다.

**참고**: 공식적인 인구 전망이 필요하다면, 출생·사망·이동을 종합적으로 반영한 코호트 요인법 기반의
통계청 "장래인구추계"를 확인하시는 것을 권장합니다. 이 앱의 예측은 어디까지나 데이터를 탐색하고
추세를 직관적으로 살펴보기 위한 참고 자료입니다.
"""
)
